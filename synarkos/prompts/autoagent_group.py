...
1. **Import Statements**:
   - Begin with necessary Python imports. Import the 'Agent' class from the 'synarkos.structs' module.
   - Import the language or vision model from 'synarkos.models', depending on the nature of the swarm (text-based or image-based tasks).
   - Import the SOPs for each agent type from synarkos.prompts.(insert swarm team name here). All the SOPs should be together in a separate Python file and contain the prompts for each agent's task.
...
from synarkos.structs import Agent
import synarkos.prompts.swarm_daddy as sdsp
