...
from synarkos.structs.swarm_router import SwarmRouter
from synarkos.utils.types import ReturnTypes
...
class YAMLConfig(BaseModel):
    ...
    agent_architecture: Optional[SwarmConfig] = None
...
def create_agents_from_yaml(
    ...
    Returns:
        ...
        - SwarmRouter (if return_type in ["auto", "swarm"] and agent_architecture defined)
    ...
    try:
        ...
        if "agent_architecture" in config:
            logger.info("Setting up swarm architecture...")
            try:
                if not isinstance(config["agent_architecture"], dict):
                    raise ValueError(
                        "agent_architecture must be a dictionary containing swarm configuration"
                    )
                ...
                missing_fields = required_fields - set(
                    config["agent_architecture"].keys()
                )
                if missing_fields:
                    raise ValueError(
                        f"SwarmRouter creation failed: Missing required fields in agent_architecture: {', '.join(missing_fields)}"
                    )
                ...
                raise ValueError(
                    ...
                    "has a valid agent_architecture section with required fields."
                )
        ...
        if return_type in ("run_swarm", "swarm"):
            if not swarm_router:
                if "agent_architecture" not in config:
                    raise ValueError(
                        "Cannot run swarm: No agent_architecture section found in YAML configuration.\n"
                        "Please add a agent_architecture section with:\n"
            ...
            try:
                if not config["agent_architecture"].get("task"):
                    raise ValueError(
                        "No task specified in agent_architecture. Please add a 'task' field "
                        "to define what the swarm should do."
                    )
                logger.info(
                    f"Running swarm with task: {config['agent_architecture']['task']}"
                )
                return swarm_router.run(
                    config["agent_architecture"]["task"]
                )
            except Exception as e:
