...
from synarkos.utils.litellm_wrapper import LiteLLM
...
BENCHMARK_CONFIG = {
    ...
    "synarkos_api_key": None,  # SynarkOS API key (will be set from env)
    "synarkos_api_base": "https://api.synarkos.ai",  # SynarkOS API base URL
...
class AOPBenchmarkSuite:
    ...
    def __init__(
        ...
        self.synarkos_api_key = os.getenv(
            "SYNARKOS_API_KEY"
    ...
    def create_real_agent(
        ...
        """
        Create a real agent for testing purposes using SynarkOS API and LiteLLM.
        ...
        try:
            ...
            if not self.synarkos_api_key:
                raise ValueError(
                    "SYNARKOS_API_KEY or OPENAI_API_KEY environment variable is required for real agent testing"
                )

            # Create LiteLLM instance for the specific model
            llm = LiteLLM(
                model_name=model_name,
                api_key=self.synarkos_api_key,
                api_base=BENCHMARK_CONFIG["synarkos_api_base"],
                temperature=BENCHMARK_CONFIG["temperature"],
                max_tokens=BENCHMARK_CONFIG["max_tokens"],
                timeout=BENCHMARK_CONFIG["timeout_seconds"],
            )

            # Create agent using proper SynarkOS pattern with LiteLLM
            agent = Agent(
