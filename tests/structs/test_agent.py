...
def flow_with_condition(mocked_llm):
    ...
    from synarkos.structs.agent import stop_when_repeats
...
def test_agent():
    ...
    with patch("synarkos.structs.agent.LiteLLM") as mock_llm:
...
class TestCreateAgentsFromYaml:
    ...
    @patch("yaml.safe_load")
    @patch(
        "synarkos.Agent.run", return_value="Task completed successfully"
    )
...
class TestAgentToolUsage:
    ...
    def test_mcp_single_url_tools(self):
        ...
        with patch(
            "synarkos.structs.agent.get_mcp_tools_sync"
        ) as mock_get_tools:
    ...
    def test_mcp_multiple_urls_tools(self):
        ...
        with patch(
            "synarkos.structs.agent.get_tools_for_multiple_mcp_servers"
        ) as mock_get_tools:
