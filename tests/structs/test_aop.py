...
from synarkos.structs.aop import (
    AOP,
    AOPCluster,
    QueueStatus,
    TaskStatus,
...
def aop_instance(real_agent, mock_fastmcp):
    ...
    with patch(
        "synarkos.structs.aop.FastMCP", return_value=mock_fastmcp
    ), patch("synarkos.structs.aop.logger"):
        aop = AOP(
...
def test_aop_initialization_all_parameters():
    ...
    with patch(
        "synarkos.structs.aop.FastMCP"
    ) as mock_fastmcp_class, patch(
        "synarkos.structs.aop.logger"
    ) as mock_logger:
        mock_mcp = Mock()
...
def test_aop_initialization_minimal_parameters():
    ...
    with patch(
        "synarkos.structs.aop.FastMCP"
    ) as mock_fastmcp_class, patch("synarkos.structs.aop.logger"):
        mock_mcp = Mock()
        mock_fastmcp_class.return_value = mock_mcp
...
def test_start_server_basic(aop_instance, mock_fastmcp):
    ...
    with patch("synarkos.structs.aop.logger") as mock_logger:
...
def test_start_server_keyboard_interrupt(aop_instance, mock_fastmcp):
    ...
    with patch("synarkos.structs.aop.logger"):
        aop_instance.start_server()
...
def test_aop_cluster_get_tools():
    ...
    with patch(
        "synarkos.structs.aop.get_tools_for_multiple_mcp_servers"
    ) as mock_get_tools:
