"""
Example script demonstrating how to fetch Figma (FIG) stock data using synarkos_tools Yahoo Finance API.
This shows the alternative approach using the existing synarkos_tools package.
"""

from synarkos import Agent
from synarkos.prompts.finance_agent_sys_prompt import (
    FINANCIAL_AGENT_SYS_PROMPT,
)
from synarkos_tools import yahoo_finance_api
from loguru import logger
import json


def get_figma_data_with_synarkos_tools():
    """
    Fetches Figma stock data using the synarkos_tools Yahoo Finance API.

    Returns:
        dict: Figma stock data from synarkos_tools
    """
    try:
        logger.info("Fetching Figma stock data using synarkos_tools...")
        figma_data = yahoo_finance_api(["FIG"])
        return figma_data
    except Exception as e:
        logger.error(f"Error fetching data with synarkos_tools: {e}")
        raise


def analyze_figma_with_agent():
    """
    Uses a SynarkOS agent to analyze Figma stock data.
    """
    try:
...
def main():
    ...
    """
    logger.info("Starting Figma stock analysis with synarkos_tools")

    try:
        # Method 1: Direct API call
        print("\n" + "=" * 60)
        print("METHOD 1: Direct synarkos_tools API call")
        print("=" * 60)

        figma_data = get_figma_data_with_synarkos_tools()
        print("Raw data from synarkos_tools:")
