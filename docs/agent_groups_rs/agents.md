...
!!! note "Modern AI Agent Framework"
    synarkos-rs is a powerful Rust framework for building autonomous AI agents powered by LLMs, equipped with robust tools and memory capabilities. Designed for various applications from trading analysis to healthcare diagnostics.
...
Here's a simple example to get you started with synarkos-rs:

```rust
use std::env;
use anyhow::Result;
use synarkos_rs::{llm::provider::openai::OpenAI, structs::agent::Agent};
...
Agents in synarkos-rs are autonomous entities that can:
...
synarkos-rs supports the Model Context Protocol (MCP), enabling agents to interact with external tools through standardized interfaces.
...
To use MCP servers with synarkos-rs, you'll need to install the appropriate tools:
...
??? question "What LLM providers are supported?"
    synarkos-rs currently supports:
...
??? question "Can I use tools without MCP?"
    Currently, synarkos-rs is designed to use the MCP protocol for tool integration. This provides a standardized way for agents to interact with external systems.
...
Contributions to synarkos-rs are welcome! Check out our [GitHub repository](https://github.com/synarkos-rs) for more information.